...
def read_answers(project_root: Path, answers_file: str | Path = ".copier-answers.yml") -> dict:
    """Read an answers file from *project_root* as a mapping.

    Defaults to the base layer's ``.copier-answers.yml``; pass a layer's own
    filename (``.copier-answers.<name>.yml``) to read that layer instead.
    ...
    """
    name = Path(answers_file).name
    path = project_root / name
    if not path.is_file():
        raise CopyRoomError(
            f"No {name} here — template commands must run from a "
